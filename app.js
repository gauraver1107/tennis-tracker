...
let currentFilter = 'season:' + getCurrentSeason().id;
...
// Persistent hidden file input — a detached input gets garbage-collected on
// iOS/Android before its 'change' event fires, which made photo picking
// silently do nothing on mobile. Keeping one input in the DOM fixes it.
let avatarFileInput = null;
let avatarTargetPlayer = null;

function ensureAvatarInput() {
  if (avatarFileInput) return avatarFileInput;
  ...
  avatarFileInput.addEventListener('change', () => {
    ...
    const player = avatarTargetPlayer;
    avatarFileInput.value = ''; // allow re-picking the same file next time
    if (!file || !player) return;
    const reader = new FileReader();
    reader.onerror = () => alert('Could not read the selected file.');
    reader.onload = (e) => {
      const img = new Image();
      img.onerror = () => alert('Could not load that image — try a different one (JPG/PNG work best).');
...
function openCropModal(img, onSave) {
  ...
  const scaleFactor = () => V / canvas.getBoundingClientRect().width; // handles narrow screens
